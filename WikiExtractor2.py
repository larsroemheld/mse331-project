...
#
acceptedNamespaces = set(['User talk'])
...
def process_data(input, output):
    ...
    timestamp = None
    id = None
    inText = False #state variable
    inSubject = False #state variable
    redirect = False
    for line in input:
        line = line.decode('utf-8')
        tag = ''
        if '<' in line: #look for tags
            m = tagRE.search(line)
            if m:
                tag = m.group(2)
